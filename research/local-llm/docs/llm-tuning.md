...
| Pinning the image tag | `latest` + `AutoUpdate=registry`, consistent with every other service. llama-swap only spawns llama-server on the **first request**, so a flag broken by an update surfaces on the next chat rather than at container start — always run a completion after `pqup`, and after the first `gpu-mode llm` that follows one, since pqup pulls the image for a stopped llama-swap too |
