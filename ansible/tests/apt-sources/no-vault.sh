#!/bin/sh
# Vault password stub for any test that reads no vaulted variable.
#
...
# a vaulted variable, and neither does tests/test-media-data-disks.yml or
# tests/test-tdarr-node-win-config.yml. It lives here for historical reasons; it
# is not specific to the apt-source harness. Do NOT point it at
# tests/test-network-interface-pinning.yml, which reads real vaulted MACs.
#
