...
echo "    5. Clear the machine-id so each clone generates a unique one on first boot:"
echo "       sudo truncate -s 0 /etc/machine-id"
echo "       (without this, every clone inherits the same /etc/machine-id, which"
echo "        collides beszel fingerprints, journald, and DHCP DUIDs)"
echo "    6. Shut down the VM and convert to template in Proxmox UI"
